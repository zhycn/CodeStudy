...
- **MappedByteBuffer**：内存映射文件缓冲区，用于高效文件I/O操作
...
| **Selector 优化** | - 设置合理的 select 超时<br>- 适时使用 wakeup()<br>- 避免空轮询 | - 平衡响应性和CPU使用率<br>- 注意线程安全                        |
| **线程模型**      | - I/O 与业务处理线程分离<br>- Boss-Worker 模型应对高并发        | - 避免在 I/O 线程执行耗时操作<br>- 根据CPU核心数设置Worker线程数 |
